...
from DuruMusic.misc import SUDOERS, db
from DuruMusic.utils.database import (
    is_active_chat,
    is_music_playing,
    is_muted,
    is_nonadmin_chat,
...
from DuruMusic.utils.decorators.language import languageCB
from DuruMusic.utils.formatters import seconds_to_min
from DuruMusic.utils.inline.play import stream_markup, telegram_markup
from DuruMusic.utils.stream.autoclear import auto_clean
from DuruMusic.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Duru.pause_stream(chat_id)
        ...
        await Duru.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
            _["admin_4"].format(mention), disable_web_page_preview=True
        )
    elif command == "Stop" or command == "End":
        await CallbackQuery.answer()
        await Duru.stop_stream(chat_id)
        ...
        await Duru.mute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await Duru.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Duru.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Duru.skip_stream(chat_id, link, video=status)
            except Exception:
            ...
            try:
                await Duru.skip_stream(chat_id, file_path, video=status)
            except Exception:
                return await mystic.edit_text(_["call_7"])
            ...
            try:
                await Duru.skip_stream(chat_id, videoid, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(_["call_7"])
            button = telegram_markup(_, chat_id)
            run = await CallbackQuery.message.reply_photo(
            ...
            try:
                await Duru.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(_["call_7"])
            if videoid == "telegram":
                button = telegram_markup(_, chat_id)
