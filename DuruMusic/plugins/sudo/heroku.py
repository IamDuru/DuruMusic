...
from DuruMusic.misc import HAPP, SUDOERS, XCB
...
async def paste_neko(code: str):
    return await Durubin(code)


@app.on_message(
    filters.command(["log", "logs", "get_log", "getlog", "get_logs", "getlogs", "og", "ols"],
                     prefixes=["/", "!", "%", ",", "-", ".", "@", "#", "l", "L"])
    & SUDOERS
)
@language
...
async def vardel_(client, message, _):
    ...
    if await is_heroku():
        ...
        if not output[0]:
            ...
            await message.reply_text(_["heroku_7"].format(check_var))
            os.system(f"kill -9 {os.getpid()} && python3 -m DuruMusic")
...
@app.on_message(filters.command(["update", "gitpull", "up", "p", "pdate"], prefixes=["/", "!", "%", ",", "-", ".", "@", "#", "u", "U"]) & SUDOERS)
@language
